...
export function MonoChartTooltipContent({
  active,
  payload,
  label,
  theme = 'dark',
  ...
  return (
    <div
      className={`px-3 py-2 rounded-xl text-xs shadow-2xl backdrop-blur-md border transition-all pointer-events-none font-sans z-50 ${
        isDark
          ? 'bg-panel/90 border-white/10 text-white shadow-black/80'
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        {payload.map((item, idx) => {
          const color = item.color || item.fill || (isDark ? 'var(--accent)' : '#000000');
